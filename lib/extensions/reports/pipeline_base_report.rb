    ...
    FundingSourceAllocation.build_temp_table do |temp_table|
      ...
      Request.connection.execute(Request.send :sanitize_sql, ["create temporary table #{report_temp_table_name}
         SELECT #{temp_select_fields} entity_name, amount_requested, amount_recommended, funding_source_allocations.program_id, funding_source_allocations.sub_program_id, 
           ...
           (select sum(rtfs.amount) from request_transaction_funding_sources rtfs, request_transactions where request_funding_source_id = request_funding_sources.id and request_transactions.id = rtfs.request_transaction_id and request_transactions.state = 'paid') paid_amount
        FROM requests, request_funding_sources #{is_type_org ? ', organizations' : ''}, #{temp_table} funding_source_allocations
        WHERE requests.id = request_funding_sources.request_id AND requests.deleted_at IS NULL AND requests.state <> 'rejected' #{(granted.to_s == '1' && start_date) ? " AND grant_agreement_at > '#{start_date.sql}' " : ''} 
              ...
              #{spending_year ? " AND spending_year = #{spending_year} " : ''}"])
